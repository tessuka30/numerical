...
function Newtonrhap() {
  ...
  function cal() {
    ...
    var xold = Number(x);
    while (err > 0.000001) {
      var fxold = math.evaluate(new AlgebraLatex().parseLatex(latex).toMath(), {
        x: xold,
      });
      var fxdif = math
        .derivative(new AlgebraLatex().parseLatex(latex).toMath(), "x")
        .evaluate({ x: xold });
      var xnew = xold - fxold / fxdif;
      dataSource.push({
        key: count,
        iteration: count,
        x: xnew,
